...
main() {
    ...
    if [ "$RUN_MOCK_ONLY" != true ]; then
        ...
        if real_network_tests_supported; then
            local _check_url="https://github.com/beCyborg/jadlis-notifications/releases/latest/download"
            local _check_bin
            ...
            local _latest_tag=""
            _latest_tag=$(curl -sI -o /dev/null -w "%{redirect_url}" --connect-timeout 5 "https://github.com/beCyborg/jadlis-notifications/releases/latest" 2>/dev/null | sed 's|.*/tag/v||')
